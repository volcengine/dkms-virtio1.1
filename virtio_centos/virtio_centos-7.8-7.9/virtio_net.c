			     ...
			     gfp_t gfp)
{
       ...
       /* the length of the rx buffer length */
        int len = vi->hdr_len + VIRTNET_RX_PAD + GOOD_PACKET_LEN;

        /* get a page as a receive buffer for rx ring */
        page = get_a_page(rq, gfp);
        if (!page)
               return -ENOMEM;

        /* max package length with skb share info */
        len = SKB_DATA_ALIGN(len) + SKB_DATA_ALIGN(sizeof(struct skb_shared_info));


       /* get a page virtual address */
       buf = page_address(page);

       /* init sg  */
        sg_init_one(rq->sg, buf + VIRTNET_RX_PAD, vi->hdr_len + GOOD_PACKET_LEN);

       /* add the buffer to virtqueue, so that device can store package to it */
